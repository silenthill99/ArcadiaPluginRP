...
public class Developpeur implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String msg, String[] args)
    {
        ...
        if (args.length != 1)
        {
            player.sendMessage(Main.getInstance().getConfig().getString("options.prefix").replace("&", "§") + " Veuillez faire /developpeur <on|off>");
            return false;
        }
        ...
        {
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "lp user " + player.getName() + " parent set développeur");
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "list");
            player.sendMessage(Main.getInstance().getConfig().getString("options.prefix").replace("&", "§") + ChatColor.YELLOW + " Vous venez de prendre votre service en tant que Développeur d'Arcadia héberge");
        } else
        {
