...
public class Administrateur implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String msg, String[] args)
    {
        ...
        {
            sender.sendMessage("Cette commande ne peut pas être exécutée via la console !");
            return false;
        }
        ...
        if (args.length != 1)
        {
            player.sendMessage(Main.getInstance().getConfig().getString("options.prefix").replace("&", "§") + " Veuillez faire /administrateur <on|off>");
            return false;
        }
        ...
        } else if (args[0].equalsIgnoreCase("on"))
        {
            ...
            player.sendMessage(Main.getInstance().getConfig().getString("options.prefix").replace("&", "§") + ChatColor.YELLOW + " Vous venez de prendre votre service en tant qu'Administrateur d'Arcadia héberge");
        } else
        {
